...
**Gotcha on rSP semantics.** Capacity math (Step 6) uses `remaining_story_points` verbatim — it's the team's recorded "this sprint's commitment." Don't try to recover apparent burndown via fallbacks; if a reportee drains rSP to 0 on Done tickets, their displayed `sprint_hours` will under-count and the Capacity verdict will look Underworked — that's a bookkeeping signal pointing at the reportee, not a math bug. See [Step 6](#step-6--assess-per-reportee-capacity) "Story-point field to use — literal rSP" for the full rule.
...
| **Remaining story points** (use `jira_field_ids.remaining_story_points` from config — discovered once per instance per Step 0) | "This sprint's commitment" after any carry-over from previous sprints. Capacity math (Step 6) uses this value verbatim — including rSP=0 on Done tickets. If the team doesn't use either field, skip capacity analysis entirely. |
...
| `changelog` filtered to status transitions, story-points field, **remaining-story-points field** (use `jira_field_ids.remaining_story_points` from config), and sprint-membership field (use `jira_field_ids.sprint` from config) | Powers the time-in-column, scope-creep, reopen, **late-sprint-addition**, and **rSP-modified-mid-sprint** signals. This is the most important field after status itself. **Scope to the sprint window**: pass `since=<sprint.start_date>` (or the MCP equivalent — `expand=changelog&startAt=…` and filter client-side if no since-parameter exists) so we don't pull history from prior sprints. For carry-over tickets the pre-sprint history can be many KB and is irrelevant to every signal we derive — all signals look at transitions inside the current sprint window only. |
...
- **Remaining story points changed this sprint** — `true` if the rSP field has a changelog entry inside the sprint window. Capture the full chronological sequence (e.g. `4 → 3 → 1 → 0`), not just first and last — the shape distinguishes one re-estimate from a multi-step burndown. Powers the **rSP modified mid-sprint** signal in Step 5.
...
### rSP modified mid-sprint

Use the **Remaining story points changed this sprint** derived value (see Step 3). If the rSP field has any changelog entry inside the sprint window, surface the ticket — and tag the owner.

**Why this signal exists.** Capacity math (Step 6) takes rSP literal: rSP=0 on a Done ticket means "no work this sprint" → 0.5h. Reportees who treat rSP as a burndown counter (drain to 0 as work completes) make themselves look Underworked despite shipping real work — the verdict is correct given the data, but the data doesn't reflect reality. The fix is to flag the bookkeeping pattern explicitly so the manager can address it with the reportee instead of misreading the Capacity overview. See [Step 6](#step-6--assess-per-reportee-capacity) "Story-point field to use — literal rSP" for the upstream rule.

**Per-ticket phrasing:**
- Single change: *"rSP changed `4 → 0` mid-sprint."*
- Multi-step burndown: *"rSP burndown `4 → 3 → 1 → 0` mid-sprint."* The shape (multiple steps, decreasing, ending at 0 on a Done ticket) is diagnostic.
- Increase rather than decrease: *"rSP raised `2 → 4` mid-sprint."* Worth a closer look — usually means a re-estimate upward (scope creep flavor) or a correction of a too-optimistic initial allocation.

**Per-reportee aggregation.** If the same reportee has rSP changes on **≥2 tickets** in the sprint, lift the signal to the Capacity overview line as a suffix: `· rSP modified on 3 tickets (burndown?)`. A 1-off rSP edit is normal bookkeeping; a 2+-ticket pattern is the bookkeeping habit worth raising.

**Don't suppress for Done tickets.** The Done+rSP=0 case is *exactly* what we want to surface — it's the load-bearing case. Other signals (Stuck, Stale, etc.) typically skip Done tickets, but this one specifically targets them.

**Skip Backlog / To Do tickets.** rSP edits before work has started are normal refinement, not bookkeeping noise. Only fire when the ticket entered an in-flight or Done status during the sprint.
...
**Don't second-guess rSP=0 on Done tickets.** A Done ticket with rSP=0 means the team marked it as "no remaining work this sprint" — usually because the work happened over multiple sprints and what landed in this sprint was just the close-out. Treating rSP=0 as 0.5h (the SP=0 mapping) is the team's recorded position. If a reportee whose Done tickets carry rSP=0 *appears* Underworked despite having shipped heavy 4-SP / 8-SP work, the issue is the reportee's bookkeeping (rSP should reflect "what this sprint cost," not be drained to 0 on close), not the math. Earlier spec versions tried to "recover" the original SP for these tickets via a Done+rSP=0 → SP fallback; that overcounted carry-over work for reportees who actually use rSP as a stable allocation. The simpler literal rule is correct.

**The bookkeeping pattern surfaces as a signal, not a math fix.** Step 5's [rSP modified mid-sprint](#rsp-modified-mid-sprint) signal flags every ticket where rSP changed in the sprint window; reportees with ≥2 such tickets get a `· rSP modified on N tickets (burndown?)` suffix on their Capacity overview line. That's how the bookkeeping habit becomes visible — the verdict stays honest, but the manager sees *why* the verdict looks light.
